...
% Geometry MUST be loaded before hyperref
...
\newfontfamily\arabicfont[Script=Arabic]{Segoe UI} % Arabic font
...
\begin{center}
  \begin{minipage}[c]{0.12\textwidth}\raggedright
    \IfFileExists{logo1.jpg}{\includegraphics[width=\linewidth]{logo1.jpg}}{}
  \end{minipage}
  ...
  \begin{minipage}[c]{0.16\textwidth}\raggedleft
    \IfFileExists{logo 2.png}{\includegraphics[width=\linewidth]{logo 2.png}}{}
  \end{minipage}
\end{center}
...
% \end{itemize}
\subsection{Model Selection: OmniASR CTC-3B}

Based on the zero-shot benchmarking results, \textbf{facebook/omniASR-LLM-3B} was selected
as the primary candidate for supervised fine-tuning on Tunisian Arabic. This choice is
motivated by three key factors:

\begin{itemize}
    \item \textbf{Competitive accuracy:} The CTC-3B variant achieves a zero-shot WER of
    \textbf{0.6795} on Tunisian data, placing it at the top of the performer cluster.
    This strong baseline provides a solid foundation for dialect-specific adaptation.

    \item \textbf{Sub-real-time inference:} With an RTF of \textbf{0.024}, the CTC-3B
    model processes audio faster than real-time, enabling responsive deployment on
    standard hardware. This is critical for dialogue system latency requirements.

    \item \textbf{Practical model scale:} At 3B parameters, the model balances
    representational capacity with memory efficiency, making it feasible to fine-tune
    on a single consumer-grade GPU (124 GB VRAM) using mixed-precision training. This
    contrasts with the 7B variant, which, while more accurate (WER 0.6720), requires
    significantly more memory and training time (787\,s vs 450\,s on 6.0\,h audio).
\end{itemize}

\subsection{Fine-Tuning Methodology}

The fine-tuning approach follows a multi-stage strategy designed to adapt the
zero-shot model to Tunisian dialect while preserving its multilingual capabilities.
The methodology consists of three phases:

\begin{enumerate}
    \item \textbf{Data preparation and validation:} Load the augmented LinTO dataset,
    apply text normalisation to produce Arabic-only transcripts, resample audio to
    16\,kHz mono, and partition into stratified train / dev / test splits.

    \item \textbf{Encoder freezing:} During the first 500 optimizer steps, the wav2vec2
    acoustic encoder --- which has already learned robust cross-lingual phonetic
    representations --- remains frozen. Only the CTC decoder is trained. This preserves
    pre-trained acoustic knowledge while allowing the decoder to specialise to Tunisian
    phonetics.

    \item \textbf{Full fine-tuning:} After encoder warm-up, all parameters are unfrozen
    and jointly optimised for the remaining steps. This allows deeper adaptation of
    acoustic patterns to dialectal variations in pronunciation, prosody, and coarticulation.
\end{enumerate}

\subsection{Experimental Setup}

\subsubsection{Hardware and Distributed Training}

Training was conducted on a single \textbf{NVIDIA GB10 GPU} with \textbf{124 GB of VRAM}.
To maximise memory efficiency and gradient accumulation, distributed training via
Fully Sharded Data Parallel (FSDP) was configured with a \textbf{stack-level granularity},
allowing parameter sharding across the single device to simulate multi-GPU training patterns.
Mixed precision training (\texttt{bfloat16}) was enabled to reduce memory consumption
without loss of numerical stability.

\subsubsection{Data Preparation and Splits}

The training dataset is the \textbf{augmented LinTO Tunisian Arabic Speech Dataset}
(\texttt{linagora/linto-dataset-audio-ar-tn-augmented}), a large-scale collection of
code-switched Tunisian speech with human-verified transcriptions. The data preparation
pipeline (as outlined in Section~\ref{sec:data-prep}) produces:

\begin{itemize}
    \item \textbf{Text cleaning:} Transcriptions are normalised to remove special markers,
    code-switching artefacts, and non-Arabic characters, resulting in pure Arabic-script
    output suitable for the CTC decoder.

    \item \textbf{Audio standardisation:} All audio is resampled to 16\,kHz mono and
    encoded in FLAC format for compact storage while maintaining lossless quality.

    \item \textbf{Stratified split:} The cleaned dataset is partitioned into 90\% training,
    5\% validation, and 5\% test sets using deterministic hashing to ensure reproducibility
    and prevent data leakage.

    \item \textbf{Parquet storage:} Data is stored in partitioned Parquet format with
    metadata fields (\texttt{corpus}, \texttt{split}, \texttt{language}) conforming to the
    omnilingual-asr \texttt{MixtureParquetStorage} schema.
\end{itemize}

\subsubsection{Hyperparameters}

The fine-tuning hyperparameters are detailed in Table~\ref{tab:omniasr_hparams}.

\begin{table}[h]
\centering
\caption{OmniASR CTC-3B Fine-Tuning Hyperparameters}
\label{tab:omniasr_hparams}
\begin{tabular}{ll}
\toprule
\textbf{Hyperparameter} & \textbf{Value} \\
\midrule
Base model              & facebook/omniASR-LLM-3B (CTC variant) \\
Dataset                 & linagora/linto-dataset-audio-ar-tn-augmented \\
Language                & Tunisian Arabic (ara\_Arab) \\
Audio sample rate       & 16,000 Hz (mono) \\
Sequence length (samples) & 480,000 (30\,s at 16\,kHz) \\
Training split          & 90\% $(\approx$ 850\,h audio) \\
Validation split        & 5\% $(\approx$ 47\,h audio) \\
Test split              & 5\% $(\approx$ 47\,h audio) \\
Optimizer               & Paged AdamW (8-bit) \\
Learning rate           & $1 \times 10^{-5}$ \\
LR scheduler            & Linear warmup + cosine decay \\
Warmup steps            & 500 (5\% of total) \\
Total optimizer steps   & 10,000 \\
Encoder freezing phase  & First 500 steps (frozen) \\
Batch size (per device) & 2 (FSDP sharding) \\
Gradient accumulation   & 2 steps \\
Effective batch size    & 4 samples \\
Mixed precision         & bfloat16 (no quantization) \\
FSDP granularity        & stack \\
Validation frequency    & Every 500 steps \\
Checkpoint frequency    & Every 1,000 steps \\
Checkpoints retained    & 3 (rolling) \\
GPU memory              & 124 GB VRAM (NVIDIA GB10) \\
\bottomrule
\end{tabular}
\end{table}
...
Given the competitive zero-shot baseline (WER 0.6795) and the large scale of the
augmented training corpus ($\approx$850\,h), the fine-tuning process is expected to
follow a predictable trajectory:

\begin{itemize}
    \item \textbf{Encoder freezing phase (steps 0--500):} The CTC decoder quickly adapts
    to Tunisian phonetics and orthography while the acoustic encoder remains stable.
    Initial loss should decrease rapidly as the decoder learns dialect-specific token
    distributions.

    \item \textbf{Joint fine-tuning phase (steps 500--10,000):} After encoder unfreezing,
    slower but more substantial improvements are expected as acoustic patterns are refined
    to match Tunisian speech characteristics. The validation loss should continue to
    decrease smoothly without signs of overfitting, given the dataset size.

    \item \textbf{Convergence target:} Based on zero-shot performance and the scale of
    available data, a target WER of \textbf{20--30\%} on the held-out test split is
    anticipated, representing a relative WER reduction of \textbf{65--75\%} compared to
    zero-shot settings. This aligns with improvements typically observed when fine-tuning
    multilingual ASR models on large in-domain corpora.

    \item \textbf{Inference characteristics:} The fine-tuned model is expected to maintain
    sub-real-time RTF ($\leq$ 0.03), ensuring compatibility with the dialogue system's
    latency budget of under 2 seconds per user turn.
\end{itemize}

\subsection{Transcript Normalisation Strategy}

To ensure consistent Arabic-script output from the CTC decoder, the following normalisation
pipeline is applied to all reference transcriptions before training:

\begin{itemize}
    \item \textbf{Code-switching handling:} Latin-script tokens (French, English) are
    either transliterated to Arabic phonetic representations using a curated transliteration
    table, or removed if no reliable mapping exists. This ensures the model sees only
    Arabic targets during training.

    \item \textbf{Diacritic handling:} Tashkeel (Arabic diacritical marks) are removed to
    reduce vocabulary size and focus the model on core consonantal structure, following
    standard practice in Arabic ASR.

    \item \textbf{Normalisation of graphical variants:} Common spelling variations
    (e.g., \ar{ي} vs \ar{ى} for final alif-maksura) are normalised to a single canonical
    form to reduce token sparsity.
\end{itemize}

\subsection{Evaluation Protocol}

After training completes, the model is evaluated on the held-out test split using the
following metrics:

\begin{itemize}
    \item \textbf{Word Error Rate (WER):} Primary metric; computed as the edit distance
    between reference and hypothesis transcriptions at the word level, normalised by
    reference length.

    \item \textbf{Character Error Rate (CER):} Secondary metric; measures character-level
    errors, providing finer-grained insight into acoustic confusions, particularly relevant
    for Arabic script.

    \item \textbf{Real-Time Factor (RTF):} Inference speed measured as the ratio of
    decoding time to audio duration. An RTF $\leq$ 0.03 confirms suitability for
    real-time dialogue.

    \item \textbf{Per-speaker and per-region analysis:} Stratified evaluation across
    different Tunisian regions and speaker demographics to assess generalisation robustness.
\end{itemize}

The final fine-tuned OmniASR-3B model will serve as the ASR backbone of the end-to-end
spoken dialogue system, providing accurate, low-latency transcription of code-switched
Tunisian speech to feed into the dialect-adapted LLM component for response generation.
%% =====================================================
%% ====================================================%% =====================================================
...
The candidate models above were chosen to cover a range of trade-offs that are important
for adapting language models to a low-resource, code-switched dialect like Tunisian Arabic.
Selection criteria include: (1) open-source availability and permissive licensing; (2)
multilingual coverage and prior evidence of Arabic or dialect competence; (3) model
scale and the resulting compute / inference cost trade-offs; and (4) community and tooling
support for fine-tuning and adapter-based adaptation.

\begin{itemize}
    \item \textbf{Llama-3.2-1B:} A compact member of the Llama family chosen as a
        lightweight baseline. Its small size enables fast iteration, low-cost
        experimentation, and early validation of preprocessing, tokenisation, and
        instruction-tuning pipelines before scaling to larger models.

    \item \textbf{Llama-3-8B:} A mid-sized general-purpose model that balances
        inference cost with representational capacity. It serves as a realistic
        production-oriented candidate where latency and cost matter but improved
        language understanding is still desired.

    \item \textbf{Labess-7B:} Included because it is the only prior work that has
        undergone continued pretraining specifically on Tunisian dialect data. Labess
        was built by further pretraining a JAIS-adapted base (\texttt{jais-adapted-7b-chat}),
        yielding a dialect-specialised initialization that captures Tunisian orthographic
        variants, code-switching behaviours, and culturally specific lexical items. 
        
    \item \textbf{SILMA-9B:} A higher-capacity open model in the candidate set to
        evaluate whether increased parameters yield measurable improvements in
        dialectal fidelity and nuanced generation for Tunisian Arabic.

    \item \textbf{Aya-Expanse-8B:} A multilingual model reported to perform well on
        a variety of languages and dialects. It was included because preliminary
        observations (and the zero-shot results shown below) indicated stronger
        baseline behaviour for dialectal adaptation, making it a promising primary
        candidate for continued pre-training and fine-tuning.
\end{itemize}

These choices allow us to measure the effect of model scale, language specialization,
and practical deployment constraints on adaptation quality. The benchmarking stage
therefore compares both raw zero-shot capability and fine-tuning efficiency across
these families to select the strongest base for Tunisian-dialect adaptation.
...
\chapter{End-to-End Pipeline Implementation \& MVP Architecture}
%% =====================================================

This chapter describes the complete end-to-end pipeline for the Tunisian Dialogue System MVP (Minimum Viable Product), integrating the fine-tuned ASR model and LLM adaptation components into a functional application accessible to end-users. The pipeline encompasses a modern React-Vite frontend with responsive mobile design and two specialized FastAPI backend modules for ASR and LLM inference with streaming capabilities.

%% =====================================================
\section{System Architecture Overview}
%% =====================================================

The MVP architecture follows a three-tier design as illustrated in Figure~\ref{fig:pipeline_architecture}:

\begin{enumerate}
    \item \textbf{Frontend Layer}: React 18 + Vite SPA with responsive Tailwind CSS + shadcn/ui components
    \item \textbf{Backend Layer}: Two independent FastAPI servers running on separate ports
    \begin{itemize}
        \item ASR Server (LINTO): Handles speech-to-text transcription with real-time streaming
        \item LLM Server: Manages inference for TounsiLM-8b fine-tuned model with token streaming
    \end{itemize}
    \item \textbf{Integration Layer}: Server-Sent Events (SSE) for real-time streaming responses to the frontend
\end{enumerate}

The decoupled architecture allows independent scaling of ASR and LLM inference servers based on demand, while CORS middleware enables seamless frontend-backend communication.

%% =====================================================
\section{Frontend: React Vite Application}
%% =====================================================

The frontend application is built with React 18 using the Vite build tool for optimal development experience and production performance. The architecture prioritizes responsive design for mobile devices while maintaining desktop usability.

\subsection{Technology Stack}

\begin{itemize}
    \item \textbf{Build Tool}: Vite (fast refresh, optimized bundling)
    \item \textbf{UI Framework}: React 18 with React Router for SPA navigation
    \item \textbf{Styling}: Tailwind CSS + shadcn/ui component library
    \item \textbf{State Management}: TanStack React Query for server state
    \item \textbf{Form Handling}: React Hook Form with validation
    \item \textbf{Responsive Design}: Mobile-first approach with Tailwind breakpoints
\end{itemize}

\subsection{Application Structure}

The MVP is structured around three main functional modules, accessible through a unified interface:

\begin{enumerate}
    \item \textbf{Voice Recording Module} (Component: \texttt{Conversation.tsx})
    \begin{itemize}
        \item Real-time audio capture using Web Audio API
        \item Displays live transcription from ASR server
        \item Streams LLM responses character-by-character for perceived responsiveness
    \end{itemize}
    \item \textbf{Text Query Module} (Component: \texttt{Conversation.tsx})
    \begin{itemize}
        \item Text input field for direct user queries
        \item Immediate LLM inference with streaming responses
        \item Conversation history display
    \end{itemize}
    \item \textbf{Media Upload Module} (Component: \texttt{MediaSummary.tsx})
    \begin{itemize}
        \item Supports WAV, MP3 audio and MP4 video files
        \item Maximum file duration: 20 minutes
        \item Multi-step streaming: transcription followed by LLM summarization
    \end{itemize}
\end{enumerate}

\subsection{User Interface Design}

The application features a Tunisian-inspired visual theme with:
\begin{itemize}
    \item Zellige-inspired background patterns for cultural resonance
    \item Arabic-friendly typography (right-to-left text support)
    \item Touch-optimized buttons and input fields for mobile devices
    \item Real-time feedback indicators (loading states, streaming indicators)
    \item Toast notifications for user feedback (via Sonner library)
\end{itemize}

%% =====================================================
\section{ASR Backend: LINTO FastAPI Server}
%% =====================================================

The ASR backend integrates the fine-tuned Whisper or Vosk-based speech recognition model with streaming transcription capabilities. Located in \texttt{mvp/linto-asr/main.py}.

\subsection{Architecture Components}

\begin{itemize}
    \item \textbf{Model}: Vosk-based offline ASR with Kaldi backend
    \item \textbf{Framework}: FastAPI with CORS middleware
    \item \textbf{Protocol}: Server-Sent Events (SSE) for streaming responses
    \item \textbf{Audio Processing}: FFmpeg for format conversion and resampling
\end{itemize}

\subsection{Transcription Pipeline}

The \texttt{/transcribe} endpoint implements the following workflow:

\begin{enumerate}
    \item \textbf{Input Handling}: Accept uploaded audio files (WebM, MP3, WAV, MP4)
    \item \textbf{Audio Conversion}: Convert to mono 16-bit PCM at 16kHz sampling rate using FFmpeg
    \begin{verbatim}
ffmpeg -y -i input.webm -ar 16000 -ac 1 -sample_fmt s16 output.wav
    \end{verbatim}
    \item \textbf{ASR Inference}: Process audio chunks through Vosk KaldiRecognizer
    \item \textbf{Streaming Response}: Stream transcription chunks in real-time
    \begin{itemize}
        \item Full sentence results with event type: \texttt{transcript\_chunk}
        \item Partial results for incremental feedback
        \item Status updates (Converting, Transcribing)
    \end{itemize}
    \item \textbf{LLM Integration}: Automatically invoke LLM server for summarization of complete transcript
\end{enumerate}

\subsection{Event Streaming Format}

Responses are formatted as Server-Sent Events (SSE) for browser compatibility:

\begin{verbatim}
data: {"type": "status", "message": "Transcribing..."}
data: {"type": "transcript_chunk", "text": "السلام عليكم"}
data: {"type": "done"}
\end{verbatim}

\subsection{Token Filtering}

The backend implements intelligent token filtering to remove:
\begin{itemize}
    \item Vosk/ASR control markers: \texttt{<START\_TIGANI>}, \texttt{<END\_TIGANI>}
    \item Special tokens: \texttt{[CLS]}, \texttt{[SEP]}, \texttt{[PAD]}, \texttt{[UNK]}
    \item LLM-specific markers from upstream services
\end{itemize}

%% =====================================================
\section{LLM Backend: TounsiLM-8b FastAPI Server}
%% =====================================================

The LLM backend serves the fine-tuned TounsiLM-8b model with streaming token generation. Located in \texttt{llm\_server.py} at the project root.

\subsection{Model Configuration}

\begin{itemize}
    \item \textbf{Base Model}: CohereLabs/aya-expanse-8b (multilingual 8B model)
    \item \textbf{Fine-tuning Method}: LoRA (Low-Rank Adaptation) with PEFT library
    \item \textbf{Target Model}: TounsiLM-8b (from \url{https://huggingface.co/alabenayed/TounsiLM-8b})
    \item \textbf{Inference Library}: HuggingFace Transformers with TextIteratorStreamer
    \item \textbf{Device Handling}: Auto GPU detection with FP16 on CUDA, FP32 on CPU
\end{itemize}

\subsection{System Prompt \& Persona}

The model operates under a carefully crafted system prompt that establishes the assistant's identity and behavioral guidelines:

\begin{quote}
\textit{`أنت "التيجاني"، مساعد ذكاء اصطناعي تونسي. جاوب بالتونسي الدارجة فقط، وبالطول المناسب للسؤال: كان يلزم قصّر، وكان يلزم فسّر أكثر. ممنوع الهلوسة أو الخروج على الموضوع.'}
\end{quote}

\textbf{Translation}:
\begin{quote}
\textit{``You are "Al-Tijani", a Tunisian AI assistant. Answer only in Tunisian dialect, with appropriate length for the question: if it needs to be short, keep it short; if it needs explanation, explain more. Hallucination and topic drift are forbidden.''}
\end{quote}

This prompt ensures:
\begin{enumerate}
    \item \textbf{Dialect Adherence}: Responses in authentic Tunisian Darija, not Modern Standard Arabic
    \item \textbf{Length Adaptation}: Model learns to calibrate response length based on question complexity
    \item \textbf{Grounding}: Explicit prohibition of hallucination and off-topic responses
    \item \textbf{Persona Consistency}: Unified identity ("Al-Tijani") across all interactions
\end{enumerate}

\subsection{Request Structure}

The LLM server accepts POST requests to \texttt{/generate} with the following schema:

\begin{verbatim}
{
  "prompt": str          # User query (min 1 character)
  "max_new_tokens": int  # Default: 220, Range: [1, 768]
  "min_new_tokens": int  # Default: 24, Range: [0, 256]
  "temperature": float   # Default: 0.5, Range: [0.0, 2.0]
...
Generation parameters are tuned for Tunisian dialect generation:
\begin{itemize}
    \item \textbf{Low temperature (0.5)}: Reduces randomness while maintaining naturalness
    \item \textbf{High top\_p (0.9)}: Broader token selection for diverse responses
    \item \textbf{Repetition penalty (1.2)}: Prevents token repetition artifacts
    \item \textbf{Max tokens (220)}: Constrains output while allowing multi-sentence responses
\end{itemize}

\subsection{Inference Pipeline}

The generation endpoint (\texttt{/generate}) executes the following steps:

\begin{enumerate}
    \item \textbf{Input Validation}: Verify prompt and generation parameters
    \item \textbf{Guardrail Check}: Apply identity-based guardrails (e.g., predefined responses for identity questions)
    \item \textbf{Chat Template Application}: Format prompt with system message and user message
    \begin{verbatim}
messages = [
...
inputs = tokenizer.apply_chat_template(
    ...
    \end{verbatim}
    \item \textbf{Token Generation}: Run model inference with TextIteratorStreamer in background thread
    \item \textbf{Stream Processing}: Yield tokens as they are generated
    \item \textbf{Stop Marker Detection}: Terminate streaming at \texttt{<END\_TIGANI>} marker
    \item \textbf{Response Assembly}: Collect tokens for final response delivery
\end{enumerate}

\subsection{Streaming Response Format}

Responses utilize Server-Sent Events format for real-time token delivery:
...
data: {"token": " عليكم"}
data: {"token": " ورحمة"}
data: {"event": "end", "response": "السلام عليكم ورحمة"}
\end{verbatim}

This allows frontend to display tokens progressively, creating a live typing effect.

\subsection{Guardrail System}

The server implements identity-based guardrails for critical prompts:

\begin{verbatim}
identity_triggers = {
    "شكونك",      # Who are you? (Tunisian)
    "شكون انت",   # Who are you?
    "من انت",     # Who are you? (MSA)
    "who are you" # English variant
}
\end{verbatim}

Matching prompts receive a consistent predefined response:
\begin{quote}
\textit{``أنا التيجاني، مساعدك التونسي. موجود هنا باش نعاونك في اللي تحب بالتونسي.''}
\end{quote}

This ensures behavioral consistency and prevents uncontrolled model drift on identity questions.
...
\hline
\textbf{Endpoint} & \textbf{Method} & \textbf{Purpose} & \textbf{Stream} \\
\hline
ASR: /transcribe & POST & Upload audio, get transcription & Yes (SSE) \\
LLM: /generate & POST & Generate response from prompt & Yes (SSE) \\
LLM: /health & GET & Health check & No \\
\hline
\end{tabular}
...
Both FastAPI servers configure CORS middleware to allow frontend requests:

\begin{verbatim}
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # Configure to specific domain in production
...
In production, \texttt{allow\_origins} should be restricted to the frontend domain.

\subsection{Request Flow for Each Functionality}

\subsubsection{Voice Recording Flow}

\begin{enumerate}
    \item Frontend captures audio using Web Audio API (MediaRecorder)
    \item Encodes as WebM with opus codec
    \item POSTs to ASR server \texttt{/transcribe} endpoint
    \item Receives transcription chunks via SSE
    \item Displays transcription in real-time
    \item Automatically invokes LLM server with full transcript
    \item Displays LLM response streaming in real-time
\end{enumerate}

\subsubsection{Text Query Flow}

\begin{enumerate}
    \item User enters text in query field
    \item Frontend POSTs to LLM server \texttt{/generate} endpoint
    \item Receives response tokens via SSE
    \item Displays tokens progressively (streaming effect)
    \item Appends response to conversation history
\end{enumerate}

\subsubsection{Media Upload Flow}

\begin{enumerate}
    \item User selects audio (WAV, MP3) or video (MP4) file (max 20 minutes)
    \item Frontend POSTs to ASR server \texttt{/transcribe} endpoint with file
    \item ASR server converts to 16kHz mono PCM WAV
    \item Streams transcription chunks via SSE
    \item Upon completion, frontend collects full transcript
    \item Automatically invokes LLM server with prompt: ``Explain clearly and shortly the key ideas and subjects:'' + transcript
    \item Displays summarized response streaming in real-time
\end{enumerate}

%% =====================================================
\section{Performance \& Optimization Considerations}
%% =====================================================

\subsection{Streaming Benefits}

The choice of Server-Sent Events (SSE) for streaming responses provides several benefits:

\begin{itemize}
    \item \textbf{Perceived Responsiveness}: Users see output appearing in real-time rather than waiting for complete response
    \item \textbf{Better UX}: Live typing effect matches natural conversation
    \item \textbf{Browser Compatibility}: SSE is widely supported, no WebSocket complexity
    \item \textbf{Graceful Degradation}: HTTP-based, works with standard proxies and load balancers
\end{itemize}

\subsection{Model Inference Optimization}

\begin{itemize}
    \item \textbf{Mixed Precision}: FP16 on GPU for faster inference and reduced memory
    \item \textbf{Background Threading}: Text generation runs in daemon thread to avoid blocking the event stream
    \item \textbf{Token Streaming}: TextIteratorStreamer yields tokens as soon as generated, not waiting for full output
    \item \textbf{Device Auto-Detection}: Automatic GPU availability detection for seamless deployment
\end{itemize}

\subsection{Audio Processing Optimization}

\begin{itemize}
    \item \textbf{Offline ASR}: Vosk model runs locally, no external API calls or latency
    \item \textbf{FFmpeg Resampling}: Hardware-accelerated audio format conversion
    \item \textbf{Chunked Processing}: Audio processed in 4000-sample chunks for low memory footprint
\end{itemize}
...
The modular architecture enables flexible deployment strategies:

\subsection{Development Deployment}

\begin{enumerate}
    \item Frontend: \texttt{npm run dev} (Vite dev server on port 5173)
    \item ASR Backend: \texttt{python main.py} (FastAPI on port 8001)
    \item LLM Backend: \texttt{python llm\_server.py} (FastAPI on port 8000)
\end{enumerate}

\subsection{Production Considerations}

\begin{itemize}
    \item \textbf{Frontend}: Build as static assets, serve via CDN or nginx
    \item \textbf{ASR Server}: Deploy with Uvicorn + Gunicorn for concurrent requests, tune max\_connections based on GPU VRAM
    \item \textbf{LLM Server}: Similar deployment as ASR, note that large model requires significant VRAM (16-24GB for 8B model with FP16)
    \item \textbf{Load Balancing}: Use reverse proxy (nginx) to distribute requests to multiple server instances
    \item \textbf{CORS Security}: Restrict \texttt{allow\_origins} to frontend domain
\end{itemize}

\subsection{Scalability Strategies}

\begin{enumerate}
    \item \textbf{Horizontal Scaling}: Deploy multiple ASR and LLM server instances behind load balancer
    \item \textbf{Vertical Scaling}: Use larger GPUs or multi-GPU setups for single server
    \item \textbf{Model Quantization}: Implement INT8 quantization for reduced memory footprint (future optimization)
    \item \textbf{Request Queuing}: Implement message queue (Redis) for non-streaming requests during peak load
\end{enumerate}
...
The MVP includes comprehensive testing infrastructure:

\subsection{Frontend Testing}

\begin{itemize}
    \item \textbf{Framework}: Vitest for unit testing
    \item \textbf{Coverage}: Component tests for all three functional modules
    \item \textbf{E2E Testing}: Manual testing of streaming flows (automated E2E suite pending)
\end{itemize}

\subsection{Backend Testing}

\begin{itemize}
    \item \textbf{Health Checks}: \texttt{/health} endpoints for monitoring
    \item \textbf{Integration Tests}: Test transcription + LLM response flow end-to-end
    \item \textbf{Performance Profiling}: Latency measurements for audio processing and token generation
\end{itemize}
...
This chapter detailed the complete end-to-end pipeline for the Tunisian Dialogue System MVP, encompassing:

\begin{enumerate}
    \item A modern, responsive React-Vite frontend optimized for mobile devices
    \item An ASR backend serving real-time transcription with streaming response
    \item An LLM backend serving TounsiLM-8b with token-level streaming
    \item Seamless integration via Server-Sent Events for perceived responsiveness
    \item Modular architecture enabling independent scaling of components
    \item Three distinct use cases: voice recording, text query, and media summarization
\end{enumerate}

The architecture prioritizes user experience through streaming responses, maintains system reliability through modular design, and enables future enhancements through clean separation of concerns. The fine-tuned models (ASR and LLM) developed in previous chapters are now fully operational within this production-ready pipeline.
