#!/usr/bin/env python3
"""Standalone runner for the Whisper Large-V3 Tunisian Arabic ASR benchmark.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the Whisper large-v3 ASR benchmark as a plain Python script."
    )
    parser.add_argument(
        "--dataset-root",
...
def main() -> int:
    ...
    # Mirror notebook: only run the two labeled_linagora splits
    notebook_splits = [s for s in labelled_splits if "linagora" in s]
    
    print(f"\nRunning only linagora splits: {notebook_splits}")
    ...
    if summary_df is not None:
        plot_wer_cer(summary_df, output_dir, model_key, mcfg["model_id"])
